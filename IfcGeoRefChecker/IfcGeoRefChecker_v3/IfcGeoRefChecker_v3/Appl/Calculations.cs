...
namespace IfcGeoRefChecker.Appl
{
    public static class Calculations
    {
        ...
        public static void GetGeoRef(bool isPosGeo, ref double lat, ref double lon, ref int? zone, ref double east, ref double north, ref bool isSouth,
            bool isEleOrtho, ref double orthoHeight, ref double ellHeight, ref double? geoidHeight,
            bool isRotGeo, ref double geoAzi, ref double gridAzi, out double convergence, out double convergenceApprox,
            out double heightScale, out double projScale, out double projScaleApprox, out double radius)
        {
            if (isPosGeo)
            {
            ...
            radius = GaussianRadiusOfCurvature(lat);
            var lon0 = ZoneToLon0(zone.Value);
            var dl = lon - lon0;
            var eastred = east - UtmFalseEasting;

            projScaleApprox = UtmScale * (1.0 + ((eastred * eastred) / (2.0 * radius * radius)));
            convergenceApprox = RadToDeg(DegToRad(dl) * Math.Sin(DegToRad(lat))); // RadToDeg(eastred /r*Math.Tan(DegToRad(lat)));// 

            geoidHeight =  geoidHeight ?? GeoidHeight(lat, lon);
            if (isEleOrtho)
            {
                ellHeight = orthoHeight + geoidHeight.Value;
            }
            else
            {
                orthoHeight = ellHeight - geoidHeight.Value;
            }
            heightScale = radius / (radius + ellHeight);
        ...
        // reduced Method to fulfill IFC requirements in GeoRefChecker
        public static void GetGeoRef(bool isPosGeo, ref double lat, ref double lon, ref int? zone, ref double east, ref double north, ref bool isSouth,
