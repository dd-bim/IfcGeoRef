...
namespace IfcGeoRefChecker
{
    ...
    public partial class Results : Window
    {
        ...
        //        this.unit = new IO.IfcReader(model).LengthUnitReader();
        ...
        //        //this.directory = model.Header.FileName.Name.Substring(0, pos);
        //    }
        //    catch(Exception ex)
        //    {
        ...
        public Results(string file, string jsonObj)
        {
            try
            {
                ...
                var rotUnits = new List<string>
            {
                ...
                foreach(var lev in json.LoGeoRef10)
                {
                    var stringID = lev.Reference_Object[0] + " = " + lev.Reference_Object[1];
                    this.Level10List.Add(stringID, lev);
                    SpatialElements10.Items.Add(stringID);
                }

                foreach(var lev in json.LoGeoRef20)
                {
                    var stringID = lev.Instance_Object[0] + " = " + lev.Instance_Object[1];
                    this.Level20List.Add(stringID, lev);
                    SiteElements20.Items.Add(stringID);
                }

                foreach(var lev in json.LoGeoRef30)
                {
                    var stringID = lev.Reference_Object[0] + " = " + lev.Reference_Object[1];
                    this.Level30List.Add(stringID, lev);
                    PlacementElements30.Items.Add(stringID);
                }

                foreach(var lev in json.LoGeoRef40)
                {
                    var stringID = lev.Reference_Object[0] + " = " + lev.Reference_Object[1];
                    this.Level40List.Add(stringID, lev);
                    PlacementElements40.Items.Add(stringID);
                }

                foreach(var lev in json.LoGeoRef50)
                {
                    var stringID = lev.Reference_Object[0] + " = " + lev.Reference_Object[1];
                    this.Level50List.Add(stringID, lev);
                    MapElements50.Items.Add(stringID);
                }
            }
            catch(Exception ex)
            {
        ...
        private void MapElements50_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            try
            {
                ...
                //if(model.SchemaVersion.ToString().Equals("Ifc2X3"))
                //{
        ...
        // --------------------------------------------------------------------------------------------------------------------------------------------------------------
        private void bt_UpdateGeoRef_Click(object sender, RoutedEventArgs e)  //nur enabling
        {
        ...
        private void bt_extractor_Click(object sender, RoutedEventArgs e)
        {
            //var reader = new IO.IfcReader(model);

            //var bldgs = reader.BldgReader();
            ////    var unit = reader.LengthUnitReader();

            ////    //LOG:
            ////    //Anzahl Buildings, Info: nur erstes Bldg in Scope

            ////    var groundWalls = reader.GroundFloorWallReader(bldgs[0]);
    ...
    private void Button_Click_1(object sender, RoutedEventArgs e)
        {
            ...
            tb_testjson.Text = json.LoGeoRef20[0].Latitude.ToString();
        }
    }
}